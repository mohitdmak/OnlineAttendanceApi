  ...
  Student.findOne({ email: req.body.email })
    .then((foundStudent) => {
      if (foundStudent !== null) {
        bcrypt
          .compare(req.body.password, foundStudent.password)
          .then((result) => {
            console.log(result);
            if (result === true) {
              ...
              res
                .status(401)
                .json({ response: "You have entered an incorrect password!" });
            }
          })
          .catch((err) => {
            console.log(err);
    ...
    .catch((err) => {
      console.log(err);
      res.status(502).json({
        response: "An error occured while finding your data from the database.",
      });
    });
};
