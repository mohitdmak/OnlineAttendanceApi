...
const get_all = (req, res) => {
  Classroom.find()
    ...
    .catch((err) => {
      console.log(err);
      res.status(502).json({
        response: "An error has occured while finding data of the classrooms.",
      });
    });
};
...
const post_create = (req, res) => {
  ...
  if (teacher) {
    ...
    res
      .status(400)
      .json({ response: "You being a student, cannot create classrooms !" });
  }
};
...
const post_join = (req, res) => {
  ...
  if (student) {
      ...
      .then((result) => {
        if (result) {
          if (result.Students.includes(student.id)) {
            ...
            res
              .status(400)
              .json({ response: "You have already joined the classroom." });
          } else {
    ...
    res
      .status(400)
      .json({ response: "You being a teacher, cannot join classrooms !" });
  }
};
...
const post_attend = (req, res) => {
  ...
  Classroom.findById(classid)
    .then((result) => {
      if (result) {
        if (result.Students.includes(id)) {
          ...
          res
            .status(200)
            .json({ response: "You can now attend your class !!!" });
        } else {
          console.log("Student is not allowed to attend the class.");
          res
            .status(400)
            .json({ response: "You must first join this class to attend !!!" });
        }
      } else {
    ...
    .catch((err) => {
      ...
      res
        .status(502)
        .json({ response: "An error has occured while attending the class." });
    });
};
